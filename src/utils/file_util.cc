...
Status SequentialFile::Back(uint64_t n)
{
    if (::fseek(fp_, -n, SEEK_CUR))
    {
        PLOG(ERROR) << "SequentialFile::Back" << filename_ << " failed: ";
        return kIOError;
    }
    return kOk;
}
...
Status WritableFile::Append(const char* data, size_t length)
{
    ...
    size_t n = ::fwrite(data, 1, length, fp_);
    while (n < length)
    {
        //size_t x = ::fwrite_unlocked(data.data() + n, 1, data.length() - n, fp_);
        size_t x = ::fwrite(data + n, 1, length - n, fp_);
        if (x == 0)
        {
            int err = ::ferror(fp_);
            if (err)
